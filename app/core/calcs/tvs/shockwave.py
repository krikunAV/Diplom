...
# Аналитически вычисленные критические Rx (аргмаксимум формул дефлаграции).
# Для Px: d/dRx(0.83/Rx − 0.14/Rx²) = 0  →  Rx* = 2·0.14/0.83
# Для Ix: d/dRx(0.06/Rx + 0.01/Rx² − 0.0025/Rx³) = 0
#   → 0.06·Rx² + 0.02·Rx − 0.0075 = 0  →  Rx* = (−0.02 + √(0.02²+4·0.06·0.0075))/(2·0.06)
_RX_CRIT_PX: float = 2.0 * 0.14 / 0.83                                         # ≈ 0.337
_RX_CRIT_IX: float = (-0.02 + math.sqrt(0.02 ** 2 + 4.0 * 0.06 * 0.0075)) / (2.0 * 0.06)  # ≈ 0.224
...
def _deflagration_px_ix(Rx: float, Vg: float, C0: float, sigma: float) -> Tuple[float, float]:
    ...
    Формулы справедливы при Rx >= Rx_crit (аргмаксимум).
    Для малых Rx используем значение в точке максимума — это консервативная
    оценка ближней зоны, обеспечивающая монотонное убывание Px и Ix с расстоянием.

    Rx_crit для Px и Ix различны (Rx_crit_Px ≈ 0.337, Rx_crit_Ix ≈ 0.224),
    поэтому применяем раздельный clamp.
    """
    Rx_px = max(Rx, _RX_CRIT_PX)
    Rx_ix = max(Rx, _RX_CRIT_IX)

    ksig = (sigma - 1.0) / sigma
    a = Vg / C0

    Px1 = (a ** 2) * ksig * (0.83 / Rx_px - 0.14 / (Rx_px ** 2))
