...
def _sum_r2L(pipes: List[Dict[str, Any]]) -> float:
    ...
    for pipe in pipes:
        ...
        L = float(pipe["L_m"])
        if r <= 0 or L <= 0:
            raise ValueError("POUO3: каждый участок должен иметь r_m > 0 и L_m > 0.")
        total += r * r * L
    return total
...
def gas_release(ctx: CalculationContext) -> None:
    """
    POUO3, 8.1: секундный массовый расход газа.
    ...
    if d <= 0:
        raise ValueError("POUO3: orifice_d_m должен быть > 0.")
    if Pg <= 0:
        raise ValueError("POUO3: Pg_Pa должен быть > 0.")
    if T <= 0 or R0 <= 0:
        raise ValueError("POUO3: T_K и R0_J_kgK должны быть > 0.")
    if mu <= 0 or psi <= 0:
        raise ValueError("POUO3: mu и psi должны быть > 0.")
    ...
    ctx.log(f"[POUO3 gas_release] F={F:.6g} m2, Vg={v_g:.6g} m3/kg, M={m_dot:.6g} kg/s")


def mass_calculation(ctx: CalculationContext) -> None:
    """
    POUO3, 8.1: масса газа до и после отсечки.
    ...
    if t_off <= 0:
        raise ValueError("POUO3: t_shutoff_s должен быть > 0.")
    if P2_kPa <= 0:
        raise ValueError("POUO3: P2_kPa должен быть > 0.")
    if rho <= 0:
        raise ValueError("POUO3: rho_gas_kg_m3 должен быть > 0.")
    if not 0 <= Z <= 1:
        raise ValueError("POUO3: Z должен быть в диапазоне [0, 1].")
...
def explosion_indoor(ctx: CalculationContext) -> IndoorNatgasResult:
    """
    POUO3, 8.1: избыточное давление взрыва в помещении.

      dP = (Pmax - P0) * ((Mg*Z)/(Vfree*rho)) * (100/Cst) * (1/Kn)
    """
    ...
    if V_free <= 0:
        raise ValueError("POUO3: V_free_m3 должен быть > 0.")
    if Pmax <= P0:
        raise ValueError("POUO3: Pmax_kPa должен быть больше P0_kPa.")
    if Kn <= 0 or Cst_percent <= 0:
        raise ValueError("POUO3: Kn и C_st_percent должны быть > 0.")
    ...
    ctx.log(f"[POUO3 indoor] dP={deltaP_kPa:.6g} kPa")

    return IndoorNatgasResult(
...
def jet_fire_stub(ctx: CalculationContext) -> None:
    """POUO3, 8.3: факельное горение фиксируется как нерассчитываемое по шаблону."""
    reason = ctx.inputs.get(
        "skip_reason",
        "Факельное горение для POUO3 не рассчитывается: время отсечки мало, критические воздействия не достигаются.",
    )
    ...
    ctx.log(f"[POUO3 jet_fire_stub] {reason}")
